...
class Window(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.log_file_path = self.team_ID_value_QL.text() + "_" + date_time + ".txt"
        ...
        self.sub_license_plate = rospy.Subscriber(
            "license_plate", String, self.licensePlate_callback
        )
        self.sub_cmd_vel = rospy.Subscriber("/R1/cmd_vel", Twist, self.cmd_vel_callback)
    ...
    def SLOT_message_received(self, license_string):
        ...
        if gndTruth == plateID:
            ...
            self.license_scores_QTW.item(plateLocation - 1, 3).setText(
                str(points_awarded)
            )
        else:
    ...
    def SLOT_timer_update(self):
        ...
        if sim_time_s > ROUND_DURATION_s:
            self.log_msg(
                "Out of time: {}sec sim time (real time: {}sec).".format(
                    sim_time_s, self.elapsed_time_s
                )
            )
            self.timer.stop()
    ...
    def stop_timer(self):
        ...
        sim_time_s = self.sim_current_time_s - self.sim_start_time_s
        self.log_msg(
            "Timer stopped: {}sec sim time (real time: {}sec).".format(
                sim_time_s, self.elapsed_time_s
            )
        )
        self.timer.stop()
