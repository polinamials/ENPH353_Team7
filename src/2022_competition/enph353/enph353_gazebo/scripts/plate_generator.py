...
with open(path + "plates.csv", "w") as plates_file:
    ...
    for i in range(0, 16):
        ...
        monospace = ImageFont.truetype(
            "/usr/share/fonts/truetype/ubuntu/UbuntuMono-R.ttf", 200
        )
        ...
        unlabelled = np.concatenate(
            (255 * np.ones(shape=[600, 600, 3], dtype=np.uint8), spot_w_plate), axis=0
        )
        cv2.imwrite(
            os.path.join(path + texture_path + "labelled/", "plate_" + str(i) + ".png"),
            labelled,
        )
        cv2.imwrite(
            os.path.join(
                path + texture_path + "unlabelled/", "plate_" + str(i) + ".png"
            ),
            unlabelled,
        )
